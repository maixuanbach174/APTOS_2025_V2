...
# Ngưỡng
THRESH_VERY_RARE = 500       # <500 khung → không ép augmentation quá nhiều
THRESH_RARE = 1000          # <1000 khung thì xoay vào nhóm “hiếm vừa”    # target ảnh cho nhóm hiếm vừa
TRHESH_MEDIUM = 1500
TRHESH_POPULAR = 2000          # max ảnh cho nhóm phổ biến
...
with open(ANNOT, newline='') as f:
    ...
    for r in rd:
        vid = r['video_id']
        if r.get('split','train').lower() != 'train': 
            continue
        c = int(r['phase_id'])
...
df.to_csv("dataset/annotations/phase_quota_config.csv", index=False)
print(df["quota"].sum())
